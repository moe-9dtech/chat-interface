...
export default function Home() {
  ...
  async function getDbMessages() {
    ...
    console.log("getDbMessages function triggered");
      ...
      <div className="flex flex-col md:flex-row md:justify-between">
        ...
        <div className="flex flex-col border-r border-[#EEEEEE] w-full sm:w-1/2 md:w-1/3 lg:w-1/4 space-y-7">
          ...
          <div className="flex flex-row bg-[#EEEEEE] py-3 mx-5 ps-4 rounded-md justify-between items-center">
            ...
            {isSearching && (
              <button className="pe-3" onClick={handleCloseSearch}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 50 50"
                  width="15"
              ...
              ? filterResults.map((singleRoom, i) => {
                  if (singleRoom.roomName) {
                    ...
                    const dpUrl = singleRoom[1]?.user?.dpurl
                      ? singleRoom[1].user.dpurl
                      : "";
                    const latestMessage =
                      singleRoom[1]?.messages &&
                      singleRoom[1].messages.length > 0
                        ? singleRoom[1].messages[
                            singleRoom[1].messages.length - 1
                          ]
                        : null; // Modified to handle case where there are no messages
                    const lastMessage =
                      latestMessage && latestMessage.sender === "admin"
          ...
          <div className="flex flex-col w-full justify-between">
            ...
            <div id="js-messages" className="flex flex-col justify-end w-full">
              ...
              {(
                ...
                rooms
                  .find((room) => {
                    if (activeIndexUsers !== -1) {
                      return room[0] === dbUsers[activeIndexUsers].roomName;
